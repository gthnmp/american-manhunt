  ...
  return (
    <div id="hero-content" className="text-white max-w-2xl h-screen flex flex-col pt-36 px-24 gap-10">
      ...
      <div className="w-36 h-10">
        <button className="bg-red-600 border-red-600 w-full h-full rounded-lg">Watch Trailer</button>
      </div>
    </div>
  );
};
...
const Episode = ({ episodeNumber, title, image }) => (
  <div className="relative w-96 h-60 flex-shrink-0">
    <div id="Information" className="absolute flex flex-col bottom-5 left-2 gap-2">
      <h2 className="text-gray-300 z-10">{`Episode ${episodeNumber}`}</h2>
      <h1 className="text-white z-10">{title}</h1>
    </div>
    <img src={image} alt={`Episode ${episodeNumber}`} className="w-full h-full object-cover" />
    <div className="dark-overlay"></div>
  </div>
);

const Episodes = () => (
  <div id="episodes" className="text-white flex flex-row flex-nowrap overflow-x-hidden items-center gap-10">
...
const Content = () => (
  <div id="content" className="absolute w-full h-screen flex">
    <HeroContent />
    <Episodes />
  </div>
);
